...
Так как недавно я перебрался с Gnome на i3wm, я стал использовать emacs-daemon и терминальную версию редактора, так что наведение красоты в этот раз пока сюда не попало.
...
** Сохранение папки с конфигурацией
...
** Переопределение =custom-file=
   ...
   Дабы не захламлять =.emacs= мусором, сохраняем его в другом месте.
